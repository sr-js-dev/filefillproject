  ...
  request('https://www.sec.gov/Archives/edgar/d' + req.query.file, function (error, response, body) {
    ...
    request('https://www.sec.gov/Archives/edgar' + str, function (error, response, body) {
      ...
      eodapi.getFundamentals(req.query.ticker).then(result => {
        ...
        var negativeInner = '';
        var str2 = '';
        var QorK = 'yearly'; // quarterly or yearly
        var balance_sheet_key_params = '';
        var cash_flow_key_params = '';
        var income_statement_key_params = '';
        if (req.query.type === 'Q') { // check GET params
          QorK = 'quarterly';
        }
        var someDate = null; // this weird date that EOD api provides

        for (var x in result['Financials']['Balance_Sheet'][QorK]){ // resolve our date with this 'weird date'
           if (result['Financials']['Balance_Sheet'][QorK][x].filing_date === req.query.date){
              someDate = x;
              break;
            }
        }
        var balance_sheet = result['Financials']['Balance_Sheet'][QorK][someDate];
        var intangibleAssets = parseInt(balance_sheet['intangibleAssets']);
        var totalLiab = parseInt(balance_sheet['totalLiab']);
        if (intangibleAssets > totalLiab) {
          str2 = "the intangible assets are greater than the total liabilities";
          positiveInner += '<li class="list-group-item">' + str2 + '</li>';
        }
        else {
          str2 = 'the total liabilities are greater than the intangible assets';
          negativeInner += '<li class="list-group-item">' + str2 + '</li>';
        }
        for (var key in balance_sheet){
          balance_sheet_key_params += '<li class="list-group-item">'+key+':'+balance_sheet[key]+'</li>';
        }
        for (var key in result['Financials']['Cash_Flow'][QorK][someDate]){
          cash_flow_key_params += '<li class="list-group-item">'+key+':'+result['Financials']['Cash_Flow'][QorK][someDate][key]+'</li>';
        }
        for (var key in result['Financials']['Income_Statement'][QorK][someDate]){
          income_statement_key_params += '<li class="list-group-item">'+key+':'+result['Financials']['Income_Statement'][QorK][someDate][key]+'</li>';
        }
        res.send(`
      <html lang="en">
        <head>
            ...
            <section class="filing">
              <div class="left_panel">
                ...
                <div class="left_panel_item keystats_div d-none">
                  <p>Key Stats</p>
                  ${balance_sheet_key_params}
                  ${cash_flow_key_params}
                  ${income_statement_key_params}
                </div>
